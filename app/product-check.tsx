...
export default function ProductCheckScreen() {
  ...
  const handleBack = useCallback(() => {
    if (step === 'results' || step === 'analyzing') {
      handleReset();
    } else {
      router.back();
    }
  }, [step, handleReset, router]);
    ...
    >
      ...
      <Animated.View entering={FadeInDown.delay(0).springify()} style={{ paddingHorizontal: 24, paddingTop: 20, paddingBottom: 12 }}>
        ...
        <Text
          style={{
            fontSize: 14,
            fontFamily: hindi ? 'Hind_400Regular' : 'PlusJakartaSans_400Regular',
        ...
        <View style={{ paddingHorizontal: 24 }}>
          <Text
            style={{
              ...
              textTransform: hindi ? 'none' : 'uppercase',
              letterSpacing: hindi ? 0 : 1.5,
              marginBottom: 12,
            }}
          >
        ...
        <View style={{ paddingHorizontal: 24 }}>
          ...
          {Object.entries(grouped).map(([category, items]) => (
            <View key={category} style={{ marginBottom: 24 }}>
              ...
              >
                {CATEGORY_EMOJI[category as ProductCategory]}{' '}
                {hindi
                  ? CATEGORY_LABEL[category as ProductCategory]?.hi
                  : CATEGORY_LABEL[category as ProductCategory]?.en}
              </Text>
              <View style={{ gap: 10 }}>
...
function ProductListItem({
  ...
  const categoryLabel = hindi
    ? CATEGORY_LABEL[product.category]?.hi
...
function AnalyzingStep({ product, hindi }: { product: Product; hindi: boolean }) {
  ...
  return (
    ...
    >
      ...
      >
        {hindi
          ? `${product.keyIngredients.length} सामग्री का विश्लेषण`
          : `Analyzing ${product.keyIngredients.length} ingredients`}
      </Text>
    </Animated.View>
  );
}
...
function ResultsStep({
  ...
  useEffect(() => {
    scoreWidth.value = withTiming(match.matchScore, { duration: 1000, easing: Easing.out(Easing.cubic) });
  }, [match.matchScore, scoreWidth]);
          ...
          >
            ...
            >
              <Text style={{ fontSize: 20, color: 'white' }}>
                {match.suitability === 'excellent' || match.suitability === 'good' ? '✓' : match.suitability === 'caution' ? '⚠' : '✕'}
              </Text>
            </View>
            <View>
              <Text
          ...
          <View style={{ padding: 20 }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 8 }}>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'baseline' }}>
                <Text style={{ fontFamily: 'Fraunces_700Bold', fontSize: 26, color: '#2D1810' }}>
                  {match.matchScore}
                </Text>
      ...
      <Animated.View entering={FadeInDown.delay(100).springify()} style={{ paddingHorizontal: 24, marginBottom: 20 }}>
        ...
        <View style={{ gap: 8 }}>
          {product.keyIngredients.map((ing, idx) => {
            const ratingColor =
              ing.rating === 'good' ? '#22C55E' : ing.rating === 'bad' ? '#EF4444' : '#EAB308';
            const ratingBg =
              ing.rating === 'good'
                ? 'rgba(34,197,94,0.1)'
                : ing.rating === 'bad'
      ...
      {improvements.length > 0 && (
        <Animated.View entering={FadeInDown.delay(250).springify()} style={{ paddingHorizontal: 24, marginBottom: 20 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            ...
            >
              {hindi ? 'अपेक्षित सुधार' : 'Expected Improvements'}
            </Text>
          </View>
          <View
            style={{
      ...
      {product.expectedTimeline.length > 0 && (
        <Animated.View entering={FadeInDown.delay(350).springify()} style={{ paddingHorizontal: 24, marginBottom: 20 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            ...
            >
              {hindi ? 'परिणाम कब दिखेंगे' : 'When to Expect Results'}
            </Text>
          </View>
          <View style={{ gap: 10 }}>
              ...
              >
                ...
                <View
                  style={{
                    flex: 1,
                    borderLeftWidth: 2,
                    borderLeftColor: 'rgba(224,120,86,0.15)',
                    paddingLeft: 14,
                  }}
                >
                  <Text
      ...
      {warnings.length > 0 && (
        <Animated.View entering={FadeInDown.delay(450).springify()} style={{ paddingHorizontal: 24, marginBottom: 20 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            ...
            >
              {hindi ? 'ज़रूरी चेतावनी' : 'Important Warnings'}
            </Text>
          </View>
          <View
            style={{
